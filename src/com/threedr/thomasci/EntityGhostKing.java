...
public class EntityGhostKing extends EntityEnemy {
	public EntityGhostKing(float x, float z, float rot, Game g) {
		super(x, z, rot, g);
		tex = TDR.getTexture("king", "img/king.png");
		bloodCol = new Vector3f(0.9f, 0.9f, 0.9f);
		maxHealth = 30;
		maxHealth += g.getLevelNum() / 2;
		health = maxHealth;
		dam += 4;
	}
	...
	public int damage(int num, int type) {
		...
		if (health <= 0 && oldHealth > 0) {
			Item i = Item.newItem(22, "kingKey");
			i.setCol(200, 190, 60);
			game.addEnt(new EntityItem(pos.x, pos.z, game, i));
		}
		return amount;
	}
}
